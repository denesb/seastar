...
static void print_no_timestamp(log_buf::inserter_iterator) {
}

static void print_boot_timestamp(log_buf::inserter_iterator it) {
    auto n = std::chrono::steady_clock::now().time_since_epoch() / 1us;
    fmt::format_to(it, "{:10d}.{:06d}", n / 1000000, n % 1000000);
}

static void print_real_timestamp(log_buf::inserter_iterator it) {
    struct a_second {
        time_t t;
        std::string s;
    };
    ...
    fmt::format_to(it, "{},{:03d}", this_second.s, ms);
}

static void (*print_timestamp)(log_buf::inserter_iterator) = print_no_timestamp;
...
logger::do_log(log_level level, log_writer& writer) {
    ...
    auto print_once = [&] (log_buf::inserter_iterator it) {
      if (local_engine) {
          fmt::format_to(it, " [shard {}]", this_shard_id());
      }
      fmt::format_to(it, " {} - ", _name);
      writer(it);
      *it = '\n';
      ++it;
    };
    ...
    if (is_ostream_enabled) {
        log_buf buf(static_log_buf.data(), static_log_buf.size());
        auto it = buf.back_insert_begin();
        fmt::format_to(it, "{} ", level_map[int(level)]);
        print_timestamp(it);
        print_once(it);
        *_out << buf.view();
    }
...
{
    try {
        lambda_log_writer writer([ex = std::move(ex)] (log_buf::inserter_iterator it) {
            fmt::format_to(it, "failed to log message: {}", ex);
        });
