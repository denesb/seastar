...
static log_buf::inserter_iterator print_no_timestamp(log_buf::inserter_iterator it) {
    return it;
}

static log_buf::inserter_iterator print_boot_timestamp(log_buf::inserter_iterator it) {
    auto n = std::chrono::steady_clock::now().time_since_epoch() / 1us;
    return fmt::format_to(it, "{:10d}.{:06d}", n / 1000000, n % 1000000);
}

static log_buf::inserter_iterator print_real_timestamp(log_buf::inserter_iterator it) {
    struct a_second {
        time_t t;
        std::string s;
    };
...
static log_buf::inserter_iterator (*print_timestamp)(log_buf::inserter_iterator) = print_no_timestamp;
