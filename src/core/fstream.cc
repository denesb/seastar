...
future<data_sink> make_file_data_sink(file f, file_output_stream_options options) {
    try {
        auto ds = data_sink(std::make_unique<file_data_sink_impl>(f, options));
        return make_ready_future<data_sink>(std::move(ds));
    } catch (...) {
        return f.close().then([ex = std::current_exception()] () mutable {
            return make_exception_future<data_sink>(std::move(ex));
        });
    }
}

future<output_stream<char>> make_file_output_stream(file f, size_t buffer_size) {
    file_output_stream_options options;
    options.buffer_size = buffer_size;
    return safe::make_file_output_stream(std::move(f), options);
}

future<output_stream<char>> make_file_output_stream(file f, file_output_stream_options options) {
