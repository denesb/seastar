...
reactor_options::reactor_options(program_options::option_group* parent_group)
    ...
    , dump_memory_diagnostics_on_alloc_failure_kind(*this, "dump-memory-diagnostics-on-alloc-failure-kind", memory::alloc_failure_kind::critical,
                program_options::enum_mapping_policy<memory::alloc_failure_kind>({{memory::alloc_failure_kind::none, "none"}, {memory::alloc_failure_kind::critical, "critical"}, {memory::alloc_failure_kind::all, "all"}}),
...
smp_options::smp_options(program_options::option_group* parent_group)
    ...
    , smp(*this, "smp", "c", {}, "number of threads (default: one per CPU)")
    , cpuset(*this, "cpuset", {}, "CPUs to use (in cpuset(7) format; default: all))")
    , memory(*this, "memory", "m", std::nullopt, "memory to use, in bytes (ex: 4G) (default: all)")
...
#else
    , allow_cpus_in_remote_numa_nodes(*this, program_options::unused{})
#endif
{
}
...
void smp::configure(const smp_options& smp_opts, const reactor_options& reactor_opts)
{
    ...
    reactor_cfg.max_networking_aio_io_control_blocks = reactor_opts.max_networking_io_control_blocks.get_value();
