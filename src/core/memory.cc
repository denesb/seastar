...
class page_list_link {
    ...
    friend void do_dump_memory_diagnostics(log_buf::inserter_iterator);
};
...
void do_dump_memory_diagnostics(log_buf::inserter_iterator it) {
    auto free_mem = cpu_mem.nr_free_pages * page_size;
    auto total_mem = cpu_mem.nr_pages * page_size;
    fmt::format_to(it, "Dumping seastar memory diagnostics\n");
    ...
    fmt::format_to(it, "index\tsize\tfree\tused\tspans\n");
