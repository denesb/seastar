...
class page_list_link {
    ...
    friend log_buf::inserter_iterator do_dump_memory_diagnostics(log_buf::inserter_iterator);
};
...
log_buf::inserter_iterator do_dump_memory_diagnostics(log_buf::inserter_iterator it) {
    ...
    for (unsigned i = 0; i < cpu_mem.small_pools.nr_small_pools; i++) {
        ...
        // We don't use pools to small to fit a free_object, so skip these, they
        // are always empty.
...
void on_allocation_failure(size_t size) {
    if (!report_on_alloc_failure_suppressed &&
        ...
        logger::lambda_log_writer writer([size] (log_buf::inserter_iterator it) {
